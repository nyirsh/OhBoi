face_up_settings = {
    name = "Token_faceup",
    url = "https://example.com/Token_faceup.png",
    removable = true,
    stackable = false,
    secret = false,
    equipment = false
...
function onRotate(spin, flip, player_color, old_spin, old_flip)
    ...
    if (rot_value > face_up_limit and old_rot_value <= face_up_limit) or
        (rot_value < face_up_limit and old_rot_value >= face_up_limit) then
...
function KTUI_CalculateTokenFace(flipped)
    ...
    if (rotC >= face_up_limit and not flipped) or (rotC <= face_up_limit and flipped) then
        is_face_up = true
