...
function takeDiceOut(tab)
    ...
    for ind, key in pairs(sortedKeys) do
        ...
        if hasGuid(objs, key) then
            ...
            getObjectFromGUID(matGuid).call("setDice", {
                diceTabTemp = diceTab,
                player = Player[sidePlayerColor]
            })
            n = n + 1
        end
    end
...
function getPoint(relativeX, relativeZ)
    local pos = Vector(getObjectFromGUID(matGuid).getPosition().x, getObjectFromGUID(matGuid).getPosition().y,
        getObjectFromGUID(matGuid).getPosition().z)
...
function setValueCallback(obj, tab)
    ...
    if (rotation.y < 2) or (88 < rotation.y and rotation.y < 92) or (178 < rotation.y and rotation.y < 182) or
        (268 < rotation.y and rotation.y < 272) or (rotation.y > 358) then
        isSquare = true
    end
    function insidef()
        obj.setValue(tab[1])
